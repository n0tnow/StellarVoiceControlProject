...
- [x] (2026-09-20, branch `feat/w19-accounts`) **W19.** Add another account while logged in: header account switcher (nickname + short address, ✓/Add/Manage + `StoreNotice`) replaces the bottom Accounts card; `ImportWallet` has two inputs (masked secret key + Name, default `Account N`, ≤24 unique) and passes `label`, duplicate → "That account is already added."; onboarding renders whenever open; check clean, app 420/0 — `backlog/w19-accounts.md`
