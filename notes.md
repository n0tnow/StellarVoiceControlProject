...
## 2026-09-20 — The Blobatar Was Never Frozen: Idle Motion Is Authored in viewBox Units
- **Symptom:** owner ran the app — "geldi ama hem çok küçük hem de hareketsiz". Two complaints, and they turned out to be one bug.
- **Measured cause (not guessed):** the animation machinery was entirely healthy — `--mo-amp` computes to `1`, the root `<g>` carries `mo-root mo-always` (the inline-SVG branch, not `<img>`), all seven idle loops report `running`, `@property` is supported, `prefers-reduced-motion` was off. But `blobatar` authors its idle layer in **viewBox units** (bob 1.1u, breathe 2.2%, saccade ~1.3u) on a 100-unit face, so the screen-pixel amplitude is whatever the face is scaled by. Driving each loop by hand through the Web Animations API: at 22 px the entire ambient layer moved **0.24–0.48 px**; at 104 px, 1.1–2.3 px. A quarter of a pixel eased over 3.4 s is invisible. **"Too small" and "motionless" were the same defect.**
- **Lesson worth keeping:** for any library that expresses motion in relative/viewBox units, the amplitude question is "how many screen pixels does this become at my size", and it has to be measured, not assumed from the fact that the animation exists. "The animation is running" and "the animation is visible" are different claims.
- **Trap avoided the second time too:** the face now has two presentations (28 px alone in the voice strip's ear, 104 px in the panel header). Moving it between those positions *in the tree* would remount it and destroy the pose morph, restart the idle phase and snap the gaze to centre — so it is one element, a direct child of `.notch`, moved and resized by CSS. Verified by element identity across four stage changes and a compact↔panel round trip, with the pose channels caught mid-transition.
- **Hard ceiling, written down:** `compact` is exactly one cutout tall (~32 px, a Rust state-table value), so a strip face tops out at 28 px and its ambient motion is inherently sub-pixel. There is no amplitude dial in the library. During a voice turn the visible motion is the **pose morph** (4× eye-height change between `speaking` and `listening`) and the blink. Making the turn face move more means a taller `compact` shell — a Rust change, not a CSS one.
- **Gaze:** wired with `useGaze({ travel, lookAt: "pointer" })` + `gaze.css`. `travel` is in viewBox units (so it scales with size on its own) and is set via the hook, never in CSS — a rule on `.mo-eyes` beats the inherited inline value, and the symptom of that mistake is identical to not wiring gaze at all.
- **Also:** the three indicator dots were removed from the strip (the face is the stage signal now); a review pass corrected the `surprised` justification (1.34 is `esx`, not `esy` — the pose is still right, being the only one that enlarges on *both* axes) and an entrance `animation` whose single `<time>` silently bound to duration, leaving the promised delay at `0s`.
- **Pointer:** `backlog/blobatar-avatar.md` (round 2 section); `app/src/notch/faceState.ts`, `app/src/notch/BlobatarFace.tsx`.
- **Status:** decided (visual result in WKWebView still unverified by the agent)

## 2026-09-20 — The Face Tracks the Native Cursor, Not the DOM Pointer; and the Panel Face Moves into the Nav Column
- **Idea (round 3):** the owner asked for four things: the panel face moved from the content header band into the top of the left nav column (and the "Ready" status label gone from `panel`), a much larger gaze excursion, gaze tracking with no click required, and the blobatar as the app icon.
- **Why a click was required — the real cause:** `lookAt: "pointer"` arms a DOM `pointermove` listener. The overlay is a non-focusable, click-through, always-on-top webview (`tauri.conf.json` `focusable: false`; `notch.rs::setup` sets `ignore_cursor_events(true)`; only `prompt` is ever `window::focus`ed). A non-key AppKit window does not deliver `mouseMoved` to the webview, so no `pointermove` ever reached the driver until a click made the window interactive enough. The project's own hover layer exists for exactly this reason (native monitor, not CSS `:hover`).
- **Fix — extend the one monitor, do not add another:** `hover.rs::observe` now forwards each `mouseMoved` sample on a new `notch_cursor` event, converted to the webview's client coordinates via the live `NSWindow` frame (`x - frame.x`, `frame.y + frame.height - y`), gated to the `compact`/`panel` states. The webview subscribes in `BlobatarFace` and calls the hook's stable `lookAt(point)`. The gaze driver is never rebuilt (a rebuilt driver snaps the eyes to centre). No second monitor, no polling watchdog.
- **Excursion is empirical, not the library's generic band:** the library documents "about 1.5–4" for its whole roster, but that is authored for the smallest head (`triangle`, ~9 units). Polaris' fitted head is radius ~34 units and its eyes do not reach the silhouette (`LIMB` 0.97, where the pupil stops and foreshortens) until travel ≈ 36. Measured through the library's own `project`: travel 10 moves the eye ~9.5 units (7.6 px at 80 px) at ~89% of its width. Panel travel went 2.8 → **10**; the face went 104 px → **80 px** in the 128 px nav column.
- **"Ready" was the strip, not a panel label:** the shell renders `.notch-content` (`StageLabel`) for every state but `prompt`, so it painted over the panel's top-left. Removed from `panel` only via `.notch-shell-panel .notch-content { display: none }`; the ~124 px header band (`.panel-body` padding override) is deleted so all four pages reclaim it.
...
- **Three conflicts, one answer each:** `app/src/lib/sfx.ts` (add/add) → #33's real module wins and the merge stub written only to make the onboarding branch compile is deleted; `app/onboarding.html` + `app/src/onboarding/main.tsx` (add/add) → one copy, the UI's, because the placeholder's job ended the moment a real entry existed; `backlog.md` (content) → both index rows kept, with the ONBOARDING-UI row's "sfx is a merge stub / the vite entry is missing" caveat dropped because it stopped describing the tree. The merge note in `main.tsx` that told a reader to do all of that was replaced by the fact: this entry is the single copy and `vite build` emits `dist/onboarding.html` from #34's `rollupOptions.input`.
- **Rehearsal mode is what keeps the gesture lessons honest:** #34 suppresses the Rust consequences of a rehearsal keystroke — while the onboarding window is visible `onboarding::is_rehearsing` is true, so `hotkey::apply` and `notch::tap` go inert. That closes the last open item in the UI report ("passing page 3 natively records a real WAV; the coordinator should decide whether first run should suppress it"). The lessons survive the silence because `useShortcutProbe` reads the native events *and* plain DOM key events on the focused window; `onboarding.rs` documents that coupling, because "simplifying" the probe onto the native events alone would silently un-gate the first run.
- **A worktree gotcha that costs twenty minutes every time:** a worktree installed before a rebase keeps the old `node_modules`. After this rebase brought #33's `uisfx` dependency, `tsc`, one test and `vite build` all failed — one root cause (module not found), no code problem. The tell is that the failures cluster on a single import rather than a single file; `npm install` at the worktree root is the fix.
