...
**Human / live.** Real Touch ID + Freighter (SEP-10, trustline, withdrawal
payment), a live SDF deposit/withdraw, and the panel on a real Mac.
...
`/info` limits + quoted fiat automatically. `demo-local` not detected yet. No
commit/PR made (coordinator commits per worktree rules).
